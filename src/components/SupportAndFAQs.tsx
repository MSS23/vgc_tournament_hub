...
const SupportAndFAQs: React.FC = () => {
  ...
  const [faqCategory, setFaqCategory] = useState<'all' | 'general' | 'tournaments' | 'technical' | 'rules' | 'registration'>('all');
  ...
  const filteredFAQs = activeCategory === 'faqs'
    ? (faqCategory === 'all' ? faqs : faqs.filter(faq => faq.category === faqCategory))
    : [];
        ...
        <div className="space-y-4">
          ...
          <div className="flex space-x-2 overflow-x-auto pb-2">
            {categories.map((category) => (
              <button
                key={category.id}
                onClick={() => setFaqCategory(category.id as any)}
                className={`px-4 py-2 rounded-full whitespace-nowrap transition-all ${
